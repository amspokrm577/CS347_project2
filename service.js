 ...
 *  POST /humans that accepts a JSON body containing a new human’s username and screen name. It returns a JSON structure reporting the ID assigned to the new human.
 *  GET /humans/:id that returns as JSON an object with the human’s screen name and username.
 *  POST /follow/:followee/:follower that adds a new following relationship to the database. It returns nothing but gives back status code 204, which means the operation silently succeeded.
 *  GET /follow/:followee that returns as JSON an array of all of the followee’s followers.
 *  DELETE /follow/:followee/:follower that adds a new following relationship to the database. It returns nothing but gives back status code 204, which means the operation silently succeeded.
 *  DELETE /humans/:id that hard-deletes the human from the database, including any following relationships the human is involved in.
 */
